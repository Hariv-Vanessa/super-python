...
class DataStream():
    def __init__(self) -> None:
        ...
        self._proc_counts: dict[str, int] = {}
    ...
    def register_processor(self, proc: DataProcessor) -> None:
        if isinstance(proc, DataProcessor):
            ...
            if proc_label:
                ...
                self._proc_counts[proc_label] = 0
            else:
    ...
    def process_stream(self, stream: list[Any]) -> None:
        def islist(data: list[Any]) -> list[Any]:
            new_stream: list[Any] = []
            for element in data:
                new_stream.append(element)
            return new_stream
    
        for item in islist(stream):
            processed: bool = False
            label = item
            for label in self._processors:
                if self._processors[label].validate(item):
                    self._processors[label].ingest(item)
                    self._proc_counts[label] += 1
                    processed = True
                    break
...
if __name__ == "__main__":
    ...
    try:
        if numeric:
            for _ in range(4):
                num_rank, data = numeric.output()
        if text:
            for _ in range(3):
                text_rank, data = text.output()
        if log:
            for _ in range(2):
                log_rank, data = log.output()
        print(f"\nConsume some elements from the data processors: {numeric.label.capitalize()} {num_rank},"
